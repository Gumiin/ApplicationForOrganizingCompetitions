...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
    {
        ...
        {
            if (user is null) return GeneralResponse.Invalid("Model is empty");

            var checkUser = await FindUserByEmail(user.Email!);
            if (checkUser != null) return GeneralResponse.Invalid("User registered already");
            ...
            if (checkAdminRole is null)
            {
                ...
                return GeneralResponse.Ok("Account created!");
            }
        ...
        {
            if (user is null) return LoginResponse.Invalid("Model is empty");

            var applicationUser = await FindUserByEmail(user.Email!);
            if (applicationUser is null) return LoginResponse.NotFound("User");

            // Verify password
            if (!BCrypt.Net.BCrypt.Verify(user.Password, applicationUser.Password))
                return LoginResponse.Invalid("Email/Password not valid");

            var getUserRole = await FindUserRole(applicationUser.Id);
            if (getUserRole is null) return LoginResponse.Invalid("User role not found");

            var getRoleName = await FindRoleName(getUserRole.RoleId);
            if (getRoleName is null) return LoginResponse.Invalid("User role not found");
            ...
            return LoginResponse.Ok("Login successfully", jwtToken, refreshToken);
        }
        ...
        {
            if (token is null) return LoginResponse.Invalid("Model is empty");

            var findToken = await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(_ => _.Token!.Equals(token.Token));
            if (findToken is null) return LoginResponse.Invalid("Refresh token is required");

            // get user details
            var user = await appDbContext.ApplicationUsers.FirstOrDefaultAsync(_ => _.Id == findToken.UserId);
            if (user is null) return LoginResponse.Invalid("Refresh token could not be generated because user not found");
            ...
            if (updateRefreshToken is null) return LoginResponse.Invalid("Refresh token could not be generated because user has not signed in");

            updateRefreshToken.Token = refreshToken;
            await appDbContext.SaveChangesAsync();
            return LoginResponse.Ok("Token refreshed successfully", jwtToken, refreshToken);
        }
        ...
        public async Task<GeneralResponse> UpdateUser(ManageUser user)
        {
            ...
            await appDbContext.SaveChangesAsync();
            return GeneralResponse.Ok("User role updated successfully");
        }
        ...
        public async Task<GeneralResponse> DeleteUser(int id)
        {
            ...
            if (user is null) return GeneralResponse.NotFound("User");
            appDbContext.ApplicationUsers.Remove(user);
            await appDbContext.SaveChangesAsync();
            return GeneralResponse.Ok("User successfully deleted");
        }
